...
const build = (
  buildBtn: HTMLElement | null,
  buildingObj: { build: (res: Resources) => boolean },
  buildText: string,
  buildIcon: string,
  buildClass: string,
) => {
...
const mapCells = document.querySelectorAll<HTMLElement>(".tile");
...
mapCells.forEach((tile) => {
  const changeSelectedCellStyles = () => {
    return tile.addEventListener("click", (e) => {
      ...
      if (tile.contains(e.target as HTMLElement)) {
...
const displayResources = () => {
  if (moneyEl) moneyEl.textContent = resources.money.toString();
  if (populationEl) populationEl.textContent = resources.population.toString();
  if (energyEl) energyEl.textContent = resources.energyCapacity.toString();
  if (happinessEl)
